...
export async function main(ns) {
  ...
  try {
    contract = JSON.parse(String(ns.args[0]));  // Use JSON parse function to decode object in a stringified argument
  } catch (e) {
    ns.tprint(`ERROR: ${e}`);
    return;
  }
  ...
  const codingContractsSolutions = [{
    ...
    },
    {
      ...
      solver: function (value) {
        ...
        const data = value.toString(2)
          .split(""); // first, change into binary string, then create array with 1 bit per index
        ...
        const parityBits = build.map((e, i) => [e, i])
          .filter(([e, _]) => e === "x")
          .map(([_, i]) => i);
    ...
    },
    {
      ...
      solver: function (data) {
        ...
        const count = (arr, val) => arr.reduce((a, v) => (v === val ? a + 1 : a), 0);
        // the count.... again ;)
        let overallParity = build.splice(0, 1)
          .join(""); // store first index, for checking in next step and fix the build properly later on
        ...
        for (let i = 0; i < sumParity; i++) {
          ...
          testArray.push(tempParity === (count(tempArray, "1") % 2).toString());
        ...
        } else if (testArray[0] === false) { // otherwise, if the overall parity is the only wrong, fix that one
    ...
    },
    {
      ...
      solver: function (data) {
        ...
        const nodes = new Array(data[0]).fill(0)
          .map(() => []);
    ...
    },
    {
      ...
      solver: function (plain) {
        let cur_state = Array.from(Array(10), () => Array(10)
          .fill(null));
    ...
    },
    {
      ...
      solver: function (data) {
        ...
        // Initial approximation: 2^(bitLength/2)
        let x = 1n << (BigInt(nbr.toString(2).length) >> 1n);
        ...
        // Compare |lower² - n| vs |upper² - n|
        const diffLower = nbr - lower * lower;
        const diffUpper = upper * upper - nbr;

        // return the closest integer (rounded)
  ...
  ns.print("Answer: " + (contract.solution.length > 50 ? String(contract.solution)
    .substring(0, 50) + "..." : contract.solution));

  if (contract.solution !== "Unsolvable") {
    contract.result = ns.codingcontract.attempt(contract.solution, contract.fileName, contract.server);
