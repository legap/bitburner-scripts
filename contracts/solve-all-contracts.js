...
export async function main(ns) {
  ...
  // Globals
  const solveContractScript = "/contracts/solve-contract.js";
  ...
  for (const contract of contracts) {
    ...
    let contractAsString;
    try {
      contractAsString = JSON.stringify(contract, bigIntReplacer);
    } catch (e) {
      ns.tprint(`failed to serialize ${contract.server}:${contract.fileName}, type: ${contract.type}, data: ${contract.data} - ${e}`);
      continue;
    }

    processID = ns.run(solveContractScript, 1, contractAsString);

    // Sleep a bit while process starts before pulling its log
    await ns.sleep(500);
    scriptLog = ns.getScriptLogs(solveContractScript, "home", contractAsString);
  ...
  // function to serialize BigInt values (used for square root contracts)
