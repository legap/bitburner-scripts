...
export async function main(ns) {
  ...
  let hackMs = 10000,
    ...
    weakenMs = 10000;
  try {
    hackMs = Math.max(1, ns.getHackTime(target));
  } catch (_) {}
  try {
    growMs = Math.max(1, ns.getGrowTime(target));
  } catch (_) {
    growMs = hackMs;
  }
  try {
    weakenMs = Math.max(1, ns.getWeakenTime(target));
  } catch (_) {
    weakenMs = hackMs;
  }
  ...
  while (true) {
    try {
      ...
      if (ramChanged && initialDeploymentDone) {
        ...
        important(
          `✓ Network RAM changed: ${lastTotalRAM.toFixed(0)}GB → ${currentTotalRAM.toFixed(0)}GB (+${ramDiff.toFixed(
            0
          )}GB)`
        );
        ...
        if (pservCount > 0 && ramDiff > 0) {
          ...
          info(
            `  Estimated: ${pservCount} purchased server${pservCount !== 1 ? "s" : ""} (~${avgIncrease.toFixed(
              0
            )}GB increase each, now ~${avgCurrentRAM.toFixed(0)}GB per server)`
          );
        }
      ...
      if (cycleCount % 10 === 0 && enableRooting) {
        ...
        info(
          `[Cycle ${cycleCount}] Scan complete: ${rootResult.totalRooted} total rooted (${rootResult.newlyRooted} new)`
        );
      ...
      if (freeRam < scriptRam) {
        error(
          `Insufficient RAM on ${pservHost}: free=${freeRam.toFixed(2)}GB need=${scriptRam.toFixed(2)}GB. Will retry.`
        );
        await ns.sleep(intervalMs);
        continue;
      }
      ...
      if (pid > 0) {
        ...
        if (!initialDeploymentDone) {
          ...
          info(
            `Monitoring mode: interval=${(intervalMs / 1000).toFixed(0)}s, scan every 10 cycles (~${(
              (intervalMs * 10) /
              60000
            ).toFixed(1)} min)`
          );
        ...
        error(
          `Failed to start ${batcher} on ${pservHost} via exec(). Possible causes: insufficient RAM, invalid args, or file missing.`
        );
        error(
          `DEBUG: ${pservHost} freeRam=${freeRam.toFixed(2)}GB scriptRam=${scriptRam.toFixed(
            2
          )}GB fileExists=${ns.fileExists(batcher, pservHost)}`
        );
      }
    } catch (e) {
