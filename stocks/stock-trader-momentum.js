...
export async function main(ns) {
  ...
  const profitTarget = Number(ns.args[2] || 0.05); // Default: 5% profit target (more achievable)
  const stopLoss = Number(ns.args[3] || 0.05); // Default: 5% stop loss
      ...
      if (longShares > 0) {
        ...
        if (shouldSell) {
          ...
          if (salePrice > 0) {
            ...
            ns.print(
              `✓ SELL ${symbol}: ${formatMoney(ns, longShares, "0.0a")} shares @ ${formatMoney(
                ns,
                salePrice,
                "$0.00a"
              )}`
            ...
            ns.print(
              `  Reason: ${reason} | Return: ${(currentReturn * 100).toFixed(
                2
              )}% | Momentum: ${positiveMovements}↑ ${negativeMovements}↓ | Profit: ${formatMoney(
                ns,
                profit,
                "$0.00a"
              )}`
      ...
      // Check if we should buy (only if under max stocks limit)
        ...
        if (sharesToBuy > 0) {
          ...
          if (purchasePrice > 0) {
            ...
            ns.print(
              `✓ BUY ${symbol}: ${formatMoney(ns, sharesToBuy, "0.0a")} shares @ ${formatMoney(
                ns,
                purchasePrice,
                "$0.00a"
              )}`
            ...
            ns.print(
              `  Momentum Buy: ${positiveMovements}↑ ${negativeMovements}↓ (riding the rally) | Swing: ${priceSwing.toFixed(
                1
              )}% | Total Cost: ${formatMoney(ns, totalCost, "$0.00a")}`
            );
...
function displayPortfolioSummary(ns, totalProfit, tradesExecuted, cycleCount) {
  ...
  if (positionCount > 0) {
    ...
    ns.print(
      `Unrealized P/L: ${formatMoney(ns, unrealizedProfit, "$0.00a")} (${((unrealizedProfit / invested) * 100).toFixed(
        2
      )}%)`
    );
  }
