/** quant-highperf-v2.js
 * High‑performance quant stock trader (v2).
 *
 * Strategy:
 * - Compute per‑symbol score:
 *     score = strength + momentum - light volatility penalty
 ...
 *
 * Usage:
 *   run stocks/quant-highperf-v2.js [max-stocks] [profit-target] [stop-loss] [refresh-ms]
 *
 * Example:
 *   run stocks/quant-highperf-v2.js 15 0.20 0.12 4000
 */

const COMMISSION = 100000;
const EXIT_NEUTRAL_THRESHOLD = 0.005;      // used only for exits
const LONG_THRESHOLD = 0.5;                // > 0.5 → long
const SHORT_THRESHOLD = 0.5;               // < 0.5 → short
const MAX_POSITION_FRACTION = 0.20;        // max 20% per stock
const MIN_TRADE_VALUE = COMMISSION * 1.5;  // tiny, just above commission
...
export async function main(ns) {
  ...
  ns.tprint("══════════════════════════════════════════════════════");
  ns.tprint(" QUANT HIGH‑PERF V2 – STARTING");
  ns.tprint("══════════════════════════════════════════════════════");
  ...
  while (true) {
    ...
    // 3) SELECTION BY SCORE (very permissive)
