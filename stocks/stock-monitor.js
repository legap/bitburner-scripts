...
export async function main(ns) {
  ...
  while (true) {
    ...
    const sessionReturn =
      startingPortfolioValue > 0 ? ((portfolioValue - startingPortfolioValue) / startingPortfolioValue) * 100 : 0;
    ...
    for (const [key, prevPos] of previousPositions) {
      if (!currentPositionKeys.has(key)) {
        ...
        ns.print(
          `[REALIZED] ${prevPos.symbol} ${prevPos.type}: ${formatMoney(ns, prevPos.profit, "$0.00a")} (${
            prevPos.returnPct > 0 ? "+" : ""
          }${prevPos.returnPct.toFixed(2)}%)`
        );
      }
    }
    ...
    ns.print(`${"─".repeat(50)}`);
    ns.print(
      `Unrealized P/L: ${formatMoney(ns, totalProfit, "$0.00a")} (${totalReturn > 0 ? "+" : ""}${totalReturn.toFixed(
        2
      )}%)`
    );
    ns.print(
      `Realized P/L: ${formatMoney(ns, realizedProfitTotal, "$0.00a")} ${
        realizedProfitTotal > 0 ? "✓" : realizedProfitTotal < 0 ? "✗" : ""
      }`
    );
    ...
    if (positions.length > 0) {
      ...
      if (has4S) {
        ns.print(
          sprintf(
            "%-6s %-5s %8s %8s %10s %10s %8s %10s %12s",
            "Symbol",
            "Type",
            "Shares",
            "Entry",
        ...
        ns.print(
          sprintf("%-6s %-5s %8s %8s %10s %10s %8s", "Symbol", "Type", "Shares", "Entry", "Current", "P/L", "Return")
        );
      }
      ...
      for (const pos of positions) {
        ...
        if (has4S) {
          ...
          const volStr = pos.volatility > 0.05 ? "HIGH" : pos.volatility > 0.02 ? "MED" : "LOW";
