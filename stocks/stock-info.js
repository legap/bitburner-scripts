...
function displayStockInfo(ns, symbol, has4S) {
  ...
  ns.tprint(
    `Spread: ${formatMoney(ns, askPrice - bidPrice, "$0.00a")} (${(((askPrice - bidPrice) / askPrice) * 100).toFixed(
      2
    )}%)`
  );
  ...
  if (longShares > 0) {
    ...
    ns.tprint(
      `  Profit/Loss: ${formatMoney(ns, longProfit, "$0.00a")} (${longReturn > 0 ? "+" : ""}${longReturn.toFixed(2)}%)`
    );
  } else {
  ...
  if (shortShares > 0) {
    ...
    ns.tprint(
      `  Profit/Loss: ${formatMoney(ns, shortProfit, "$0.00a")} (${shortReturn > 0 ? "+" : ""}${shortReturn.toFixed(
        2
      )}%)`
    );
  } else {
...
function displayAllStocks(ns, has4S) {
  ...
  // Header
  if (has4S) {
    ns.tprint(sprintf("%-6s %10s %10s %8s %8s %8s", "Symbol", "Ask", "Bid", "Forecast", "Position", "P/L"));
  } else {
    ns.tprint(sprintf("%-6s %10s %10s %8s %8s", "Symbol", "Ask", "Bid", "Position", "P/L"));
  }
  ns.tprint(`${"─".repeat(70)}`);
  ...
  if (totalInvested > 0) {
    ...
    ns.tprint(
      `Total P/L: ${formatMoney(ns, totalProfit, "$0.00a")} (${totalReturn > 0 ? "+" : ""}${totalReturn.toFixed(2)}%)`
    );
  }
}
