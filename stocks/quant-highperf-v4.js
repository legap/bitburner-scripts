/** quant-highperf-v4.js
 * High‑performance quant stock trader (v4).
 *
 * Changes vs v3:
 * - Stricter entry thresholds: long > 0.60, short < 0.40
 * - Forecast-driven exits (exit when signal flips)
 * - Default profitTarget 12%, stopLoss 6%
 * - Summary includes count of open long/short positions
 *
 * Strategy:
 * - Score per symbol: strength + momentum - light volatility penalty
 * - Capital allocated proportional to scores
 * - Long if forecast > LONG_THRESHOLD, short if forecast < SHORT_THRESHOLD
 * - Max 20% of total capital per stock
 * - Low min trade threshold so it actually trades
 *
 * Usage:
 *   run stocks/quant-highperf-v4.js [max-stocks] [profit-target] [stop-loss] [refresh-ms]
 *
 * Example:
 *   run stocks/quant-highperf-v4.js 15 0.12 0.06 4000
 */

const COMMISSION = 100000;
const LONG_THRESHOLD = 0.52;            // bullish
const SHORT_THRESHOLD = 0.48;           // bearish
...
export async function main(ns) {
  ...
  const profitTarget = Number(ns.args[1] ?? 0.12);
  const stopLoss     = Number(ns.args[2] ?? 0.06);
  ...
  ns.tprint("══════════════════════════════════════════════════════");
  ns.tprint(" QUANT HIGH‑PERF V4 – STARTING");
  ns.tprint("══════════════════════════════════════════════════════");
  ...
  while (true) {
    ...
    // 1) EXIT LOGIC (forecast-driven + target/stop)
    for (const sym of symbols) {
      ...
      // Long exit: profit, stop, or forecast no longer strongly bullish
      if (longShares > 0) {
        ...
        const exit =
          pct >= profitTarget ||
          pct <= -stopLoss ||
          forecast < LONG_THRESHOLD;   // signal weakened
      ...
      // Short exit: profit, stop, or forecast no longer strongly bearish
      if (shortShares > 0 && canShort) {
        ...
        const exit =
          pct >= profitTarget ||
          pct <= -stopLoss ||
          forecast > SHORT_THRESHOLD;  // signal weakened
...
async function rebalanceLong(ns, sym, currentShares, targetValue, cash) {
  ...
  if (diff > 0) {
    ...
    if (buy > 0) {
      ...
      if (p > 0) ns.print(`REB_LONG BUY  ${sym} eff=${fmt(ns, buy)} price=${fmt(ns, p)}`);
    }
  } else {
    const sell = -diff;
    if (sell > 0) {
      const p = ns.stock.sellStock(sym, sell);
      if (p > 0) ns.print(`REB_LONG SELL ${sym} eff=${fmt(ns, sell)} price=${fmt(ns, p)}`);
    }
  }
}
  ...
  try {
    if (diff > 0) {
      ...
      if (amt > 0) {
        ...
        if (p > 0) ns.print(`REB_SHORT OPEN  ${sym} eff=${fmt(ns, amt)} price=${fmt(ns, p)}`);
      }
    } else {
      const amt = -diff;
      if (amt > 0) {
        const p = ns.stock.sellShort(sym, amt);
        if (p > 0) ns.print(`REB_SHORT COVER ${sym} eff=${fmt(ns, amt)} price=${fmt(ns, p)}`);
      }
    }
  } catch {
    canShort = false;
