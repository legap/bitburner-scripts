...
export async function main(ns) {
  ...
  for (const pos of positions) {
    const profitStr =
      pos.profit > 0 ? `+${formatMoney(ns, pos.profit, "$0.00a")}` : formatMoney(ns, pos.profit, "$0.00a");
    ...
    ns.tprint(
      `${emoji} ${pos.symbol} ${pos.type}: ${formatMoney(ns, pos.shares, "0.0a")} shares @ ${formatMoney(
        ns,
        pos.entryPrice,
        "$0.00a"
      )} → ${formatMoney(ns, pos.exitPrice, "$0.00a")}`
    );
