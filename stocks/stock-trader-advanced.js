...
function formatMoney(ns, v, f) {
  try {
    ...
    return (v < 0 ? "-$" : "$") + n.toFixed(f.includes(".00") ? 2 : f.includes(".000") ? 3 : 0) + u[i];
  }
}

const EXIT_THRESHOLD = 0.02; // Exit if forecast moves within 2% of neutral
const COMMISSION = 100000; // Transaction commission
const MAX_POSITION_SIZE = 0.1; // Max 10% of portfolio per stock
...
export async function main(ns) {
  ...
  ns.print(
    `Investment per Stock: ${formatMoney(ns, investmentPerStock, "$0.00a")} (after ${formatMoney(
      ns,
      COMMISSION,
      "$0.00a"
    )} commission)`
      ...
      if (longShares > 0) {
        ...
        if (shouldExit) {
          ...
          if (salePrice > 0) {
            ...
            ns.print(
              `✓ SELL LONG ${symbol}: ${formatMoney(ns, longShares, "0.0a")} @ ${formatMoney(ns, salePrice, "$0.00a")}`
            );
            ns.print(
              `  Reason: ${reason} | Net Return: ${(netReturnPct * 100).toFixed(2)}% | Profit: ${formatMoney(
                ns,
                profit,
                "$0.00a"
              )}`
        ...
        if (shouldExit) {
          try {
            ...
            if (salePrice > 0) {
              ...
              const reason = hitProfitTarget ? "PROFIT TARGET" : hitStopLoss ? "STOP LOSS" : "FORECAST";
              ns.print(
                `✓ CLOSE SHORT ${symbol}: ${formatMoney(ns, shortShares, "0.0a")} @ ${formatMoney(
                  ns,
                  salePrice,
                  "$0.00a"
                )}`
      ...
      if (forecast > LONG_THRESHOLD) {
        ...
        if (sharesToBuy > 0) {
          ...
          if (purchasePrice > 0) {
            ...
            ns.print(
              `✓ BUY LONG ${symbol}: ${formatMoney(ns, sharesToBuy, "0.0a")} @ ${formatMoney(
                ns,
                purchasePrice,
                "$0.00a"
              )}`
            ...
            ns.print(
              `  Forecast: ${(forecast * 100).toFixed(1)}% | Confidence: ${(confidence * 100).toFixed(
                1
              )}% | Cost: ${formatMoney(ns, totalCost, "$0.00a")}`
            );
        ...
        if (sharesToShort > 0) {
          try {
            ...
            if (purchasePrice > 0) {
              ...
              ns.print(
                `✓ SHORT ${symbol}: ${formatMoney(ns, sharesToShort, "0.0a")} @ ${formatMoney(
                  ns,
                  purchasePrice,
                  "$0.00a"
                )}`
...
function displayAdvancedSummary(ns, totalProfit, tradesExecuted, biggestWin, biggestLoss) {
  ...
  ns.print(`${"─".repeat(70)}`);
  ns.print(
    `Unrealized P/L: ${formatMoney(ns, unrealizedProfit, "$0.00a")} (${
      invested > 0 ? ((unrealizedProfit / invested) * 100).toFixed(2) : "0.00"
    }%)`
  );
  ...
  if (tradesExecuted > 0) {
    ns.print(
      `Best Trade: ${biggestWin > 0 ? formatMoney(ns, biggestWin, "$0.00a") : "$0.00 (no profitable trades yet)"}`
    );
