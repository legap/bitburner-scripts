...
export async function main(ns) {
  ...
  ns.tprint(`${"═".repeat(70)}`);
  ns.tprint(
    `Collecting price data over ${cyclesToRun} cycles (${((cyclesToRun * refreshRate) / 1000).toFixed(0)} seconds)...`
  );
...
function displayRecommendations(ns, rec, has4S) {
  ...
  ns.tprint(
    `Strong Buy: ${rec.strongBuy.length} | Buy: ${rec.buy.length} | Hold: ${rec.hold.length} | Avoid: ${rec.avoid.length}`
  );
  ...
  if (rec.strongBuy.length > 0) {
    ...
    for (const stock of rec.strongBuy) {
      ns.tprint(
        `${stock.symbol.padEnd(6)} @ ${formatMoney(ns, stock.askPrice, "$0.00a").padEnd(8)} | ` +
      ...
      if (has4S) {
        ...
        ns.tprint(
          `         📊 Forecast: ${forecastPct}% ${forecastDir} | ` +
