import {
  getConfiguration, formatRam, formatMoney, formatNumber,
} from "./helpers.js";
...
export function main(ns) {
  ...
  function createServerEntry(serverName) {
    ...
    return (
      ...
      + `${rooted ? " rooted" : ""}">${serverName}</a>${
        ...
        shouldBackdoor
          ? ` <span class="${factionServers.includes(serverName) ? "faction " : ""}backdoor">[<a>backdoor</a>]</span>`
          : ""
      }`
      + ` ${contracts.map((c) => `<span class="cct" title="${c}">@</span>`).join("")}${
        showStats
          ? " <span class=\"serverStats\">... "
            + `Money: ${`${
              server.moneyMax ?? 0 > 0 ? `${formatMoney(server.moneyAvailable ?? 0, 4, 1).padStart(7)} / ` : ""
            }${formatMoney(server.moneyMax ?? 0, 4, 1).padStart(7)} `.padEnd(18)}${`Sec: ${formatNumber(
              server.hackDifficulty ?? 0,
              0,
              0
            ).padStart(3)}/${formatNumber(server.minDifficulty ?? 0, 0, 0)} `.padEnd(13)}RAM: ${formatRam(
              server.maxRam ?? 0
            )
              .replace(" ", "")
              .padStart(6)}${
  ...
  function buildOutput(parent = servers[0], prefix = ["\n"]) {
    ...
    if (showStats) {
      ...
      const expectedLength = parent.length
        + 2 * prefix.length
  ...
  function ordering(serverA, serverB) {
    ...
    if (orderNumber == 0) orderNumber = getServerInfo(serverB).purchasedByPlayer - getServerInfo(serverA).purchasedByPlayer;
    // Hack: compare just the first 2 chars to keep purchased servers in order purchased
    if (orderNumber == 0) orderNumber = serverA.slice(0, 2).toLowerCase().localeCompare(serverB.slice(0, 2).toLowerCase());
    return orderNumber;
  }
  ...
  let servers = ["home"];
  let parentByIndex = [""];
  const routes = { home: "home" }; // Command to connect to this server
  for (const server of servers) {
    for (const oneScanResult of ns.scan(server).sort(ordering)) {
  ...
  doc
    ...
    .forEach((serverEntry) => serverEntry.addEventListener("click", setNavCommand.bind(null, routes[serverEntry.childNodes[0].nodeValue])));
  doc
    .querySelectorAll(".serverscan.new .backdoor")
    .forEach((backdoorButton) => backdoorButton.addEventListener(
      "click",
