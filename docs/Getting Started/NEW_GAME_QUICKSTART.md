...
2. **Install Scripts**
   ...
   **💡 Advanced Option**: For faster development with instant sync, see
   [Remote API Setup Guide](docs/REMOTE_API_SETUP.md)
...
**💡 Note**: In quiet mode, you'll still see important events like "✓ Rooted: server-name" but routine status messages
are hidden.
...
If you've purchased **Formulas.exe** ($5 billion, permanent across resets), you get access to **perfect accuracy**
scripts with zero estimation error:
...
**When to Use**: After purchasing Formulas.exe, use the `f-` prefix versions for perfect accuracy. They check for
Formulas.exe and show an error if not installed.

**More Info**: See [docs/FORMULAS_ENHANCED_SCRIPTS.md](docs/FORMULAS_ENHANCED_SCRIPTS.md) and
[docs/SCRIPTS_USING_FORMULAS.md](docs/SCRIPTS_USING_FORMULAS.md)

---

### RAM Sharing for Faction Reputation

If you're grinding **faction reputation** (for augmentations), you can share your free RAM for a reputation multiplier
bonus:
...
**Real Example**: With 68 servers (25 purchased + network servers), sharing 7,176 threads gives **1.356x reputation
multiplier** (35.6% bonus).
...
3. **🔮 Use formula scripts if you have Formulas.exe** - Perfect accuracy with f-profit-scan-flex.js and
...
1. **After getting this guide working**: Read [docs/GETTING_STARTED.md](docs/GETTING_STARTED.md) for deeper
   understanding
...
**Remember**: The faster you set up automation, the faster you progress. Don't overthink it - run the scripts, monitor
the output, and scale up!
