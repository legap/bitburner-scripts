...
export async function main(ns) {
  ...
  try {
    ...
    ns.tprint(
      `Current Money: ${formatMoney(ns, currentMoney, "$0.00a")} (${((currentMoney / maxMoney) * 100).toFixed(1)}%)`
    );
    ...
    for (const threads of [1, 5, 10, 25, 50, 100]) {
      ...
      ns.tprint(
        `${threads} hack threads: ${formatMoney(ns, moneyPerSecond, "$0.00a")}/s, ${formatMoney(
          ns,
          moneyPerMinute,
          "$0.00a"
        )}/min, ${formatMoney(ns, moneyPerHour, "$0.00a")}/hr`
      );
    }
    ...
    if (curSec > minSec + 0.5 || currentMoney < maxMoney * 0.95) {
      ...
      for (const threads of [1, 5, 10, 25, 50, 100]) {
        ...
        ns.tprint(
          `${threads} hack threads: ${formatMoney(ns, moneyPerSecond, "$0.00a")}/s (+${improvement.toFixed(
            1
          )}% vs current)`
        );
      }
